...
class ResConfigSettings(models.TransientModel):
    ...
    @api.model
    def get_values(self):
        """Load boolean value from ir.config_parameter."""
        ...
        res["website_login_redirect_enabled"] = enabled_param == "1"
        return res

    def set_values(self):
        """Save boolean value to ir.config_parameter."""
        super().set_values()
        enabled_value = "1" if self.website_login_redirect_enabled else "0"
        self.env["ir.config_parameter"].sudo().set_param(
            "website_user_login_redirect_custom.enabled", enabled_value
        )
